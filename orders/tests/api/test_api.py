...
def test_list_shops(client, shop_factory, buyer):
    ...
    #     assert shop['name'] == test_shops[i].name
    # Пыталась сделать так, но магазины сохраняются/выдаются
    # не по порядку, поэтому взаимодействите по id не получается

    for test_shop in test_shops:
        shop_id = Shop.objects.get(id=test_shop.id)
        shop_name = Shop.objects.get(name=test_shop.name)
...
def test_list_categories(client, category_factory, buyer):
    ...
    for test_category in test_categories:
        category_id = Category.objects.get(id=test_category.id)
        category_name = Category.objects.get(name=test_category.name)
...
def test_list_products(client, product_factory, buyer):
    ...
    for test_product in test_products:
        product_id = Product.objects.get(id=test_product.id)
        product_name = Product.objects.get(name=test_product.name)
